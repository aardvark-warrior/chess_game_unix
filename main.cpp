    ...
    if (choice.length() != 1) {
      ...
      case 'M': case 'm': {
        ...
        // And make the move
          ...
          ////////////////////////////////////////////////////
          // TODO: Add exception handling here so that      //
          // an exception resulting from an illegal move is //
          // handled as specified in the project writeup.   //
          ////////////////////////////////////////////////////
          try {
          game.make_move(std::make_pair(argument[0], argument[1]),
                   std::make_pair(argument[2], argument[3]));
          } catch (const Chess::Exception& ex) {
            std::cerr << "Could not make move: " << ex.what() << std::endl;
          }
        }
        break;
      }
